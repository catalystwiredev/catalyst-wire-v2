...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (loginRes?.error) { router.push("/login"); } else { router.push("/register/success"); router.refresh(); }
    } catch {
      ...
      <div style={{ width: "100%", maxWidth: 440 }}>
        ...
        <form onSubmit={handleSubmit} style={{ background: "var(--bg-card)", border: "1px solid var(--border-medium)", borderRadius: 16, padding: 28 }}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Angel Gomez" required autoComplete="name" style={input}
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" required autoComplete="email" style={input}
              onFocus={e => (e.target.style.borderColor = "var(--accent)")} onBlur={e => (e.target.style.borderColor = "var(--border-medium)")}/>
          </div>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ position: "relative" }}>
              <input type={showPw ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} placeholder="Min. 8 characters" required autoComplete="new-password" style={{ ...input, paddingRight: 44 }}
                onFocus={e => (e.target.style.borderColor = "var(--accent)")} onBlur={e => (e.target.style.borderColor = "var(--border-medium)")}/>
            ...
            {password.length > 0 && (
              <div style={{ marginTop: 8 }}>
                <div style={{ display: "flex", gap: 4 }}>
                  {[1,2,3,4].map(n => (
                    <div key={n} style={{ flex: 1, height: 3, borderRadius: 2, background: n <= pwStrength ? strengthColor : "var(--border-medium)", transition: "background 0.2s" }}/>
                  ))}
                </div>
