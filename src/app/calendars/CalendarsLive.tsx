...
import { Calendar, FlaskConical, BarChart2, Rocket } from "lucide-react";
...
interface CalendarData { earnings: unknown[]; fdaApps: unknown[]; ipoFilings: unknown[]; }
  ...
  return (
    <>
      ...
      <div style={{ marginBottom: 32 }}>
        <SectionHeader icon={BarChart2} title="Upcoming Earnings" count={earnings.length} color="#f59e0b" />
        {earnings.length === 0
          ? <EmptyState label="No upcoming earnings on the calendar." icon={BarChart2} color="#f59e0b" />
          : <EarningsTable earnings={earnings as Parameters<typeof EarningsTable>[0]["earnings"]} />}
      </div>

      <div style={{ marginBottom: 32 }}>
        <SectionHeader icon={FlaskConical} title="FDA Applications" count={fdaApps.length} color="#f472b6" />
        {fdaApps.length === 0
          ? <EmptyState label="No active FDA applications." icon={FlaskConical} color="#f472b6" />
          : <FDAApplicationsTable applications={fdaApps as Parameters<typeof FDAApplicationsTable>[0]["applications"]} />}
      </div>

      <div>
        <SectionHeader icon={Rocket} title="IPO & S-1 Filings" count={ipoFilings.length} color="#a78bfa" />
        {ipoFilings.length === 0
          ? <EmptyState label="No recent IPO filings." icon={Rocket} color="#a78bfa" />
          : <SecFilingsTable filings={ipoFilings as Parameters<typeof SecFilingsTable>[0]["filings"]} />}
      </div>

      <span style={{ display: "none" }} aria-hidden><Calendar /></span>
    </>
  );
}
