...
async function getCOTData(session: any) {
  try {
...
export default async function COTPage() {
  ...
  const data = await getCOTData(session);
    ...
    <div style={{ padding: "32px 24px", maxWidth: 1200, margin: "0 auto" }}>
      <ScrollReveal>
        <DataPageHeader
          ...
          description="CFTC institutional positioning — non-commercial (speculator) vs commercial (hedger) net positions. Refreshed weekly with live updates."
          stats={[
            { label: "Markets Tracked", value: rows.length, color: "#a78bfa" },
            { label: "Net Long", value: netLongs, color: "#00e676" },
            { label: "Net Short", value: netShorts, color: "#ff4d4d" },
            { label: "Total Open Interest", value: totalOI > 1e6 ? `${(totalOI / 1e6).toFixed(1)}M` : `${(totalOI / 1e3).toFixed(0)}K`, color: "#0090f0" },
          ]}
          isPremium={data.premium}
          upgradeHref="/pricing"
          upgradeLabel="Unlock All Markets + Full History"
