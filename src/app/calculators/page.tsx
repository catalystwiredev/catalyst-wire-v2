...
const resultStyle: React.CSSProperties = { background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: 10, padding: "16px 20px", marginTop: 16 };
...
function PnL() {
  ...
  const e = parseFloat(entry), x = parseFloat(exit), q = parseFloat(qty), f = parseFloat(fees) || 0;
  const gross = (x - e) * q;
  const net   = gross - f;
  const pct   = ((x - e) / e) * 100;
  return (
...
function DCA() {
  ...
  const a = parseFloat(amount), m = parseInt(months), ip = parseFloat(initPrice), fp = parseFloat(finalPrice);
  const avgPrice  = (ip + fp) / 2;
  const totalInvested = a * m;
  const totalShares   = a / ip + (a / fp) * (m - 1);
...
function PositionSize() {
  ...
  const c = parseFloat(capital), r = parseFloat(risk) / 100, e = parseFloat(entry), s = parseFloat(stop);
  const riskAmount  = c * r;
  const stopDist    = Math.abs(e - s);
  const shares      = Math.floor(riskAmount / stopDist);
  const posValue    = shares * e;
  const positionPct = (posValue / c) * 100;
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
...
function CompoundGrowth() {
  ...
  const p = parseFloat(principal), r = parseFloat(rate) / 100, y = parseInt(years), m = parseFloat(monthly);
...
function DividendYield() {
  ...
  const [years,    setYears]    = useState("10");
  const p = parseFloat(price), a = parseFloat(annual), s = parseFloat(shares), g = parseFloat(growth) / 100, y = parseInt(years);
  const yieldPct       = (a / p) * 100;
  const annualIncome   = a * s;
  const futureDiv      = a * Math.pow(1 + g, y);
  const futureYield    = (futureDiv / p) * 100;
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
...
function CoveredCall() {
  ...
  const [shares,     setShares]     = useState("100");
  const sp = parseFloat(stockPrice), pr = parseFloat(premium), st = parseFloat(strike), sh = parseFloat(shares);
  const premiumIncome    = pr * sh;
  const maxProfit        = (st - sp + pr) * sh;
  const breakeven        = sp - pr;
  const returnOnStock    = (pr / sp) * 100;
  const cappedAt         = st;
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
  ...
  return (
    <div style={{ maxWidth: 960, margin: "0 auto", padding: "32px 24px 72px" }}>
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ fontSize: 14, color: "var(--text-secondary)" }}>Real-time calculators for trading analysis, risk management, and investment planning.</p>
      </div>

      {/* Tab bar */}
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 28 }}>
        {CALCS.map(({ id, label, icon: Icon }) => (
          <button key={id} onClick={() => setActive(id)} style={{ display: "flex", alignItems: "center", gap: 7, padding: "8px 16px", borderRadius: 8, background: active === id ? "var(--accent)" : "var(--bg-elevated)", color: active === id ? "#fff" : "var(--text-secondary)", border: `1px solid ${active === id ? "transparent" : "var(--border-medium)"}`, fontSize: 13, fontWeight: 500, cursor: "pointer", transition: "all 0.15s" }}>
            <Icon size={13}/> {label}
          </button>
        ))}
      </div>
