 ...
 * Secret: TIINGO-API-KEY (from Azure Key Vault)
...
async function authHeader(): Promise<Record<string, string>> {
  const k = await getSecret("TIINGO-API-KEY");
...
export async function getTiingoIEX(symbol: string): Promise<TiingoIEX | null> {
  try {
    ...
    const res = await fetch(`${BASE}/iex/${symbol.toUpperCase()}`, { 
      headers, 
      next: { revalidate: 30 } 
    });
    if (!res.ok) { 
      console.error("[tiingo] iex HTTP", res.status); 
      return null; 
    }
...
export async function getTiingoIntraday(symbol: string, opts: {
  ...
  try {
    ...
    const res = await fetch(`${BASE}/iex/${symbol.toUpperCase()}/prices?${params}`, { 
      headers, 
      next: { revalidate: 60 } 
    });
    if (!res.ok) return [];
...
export async function getTiingoFundamentalsDaily(symbol: string): Promise<TiingoFundamentals | null> {
  try {
    const token = await getSecret("TIINGO-API-KEY");
    const res = await fetch(`${BASE}/tiingo/fundamentals/${symbol.toUpperCase()}/daily?token=${token}`, { 
