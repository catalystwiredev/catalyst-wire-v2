/**
 * CFTC Commitment of Traders (COT) reports — public Socrata API, NO key required.
 * Source: https://publicreporting.cftc.gov
 *
 * Dataset `jun7-fc8e` = Legacy Futures + Options Combined report:
 *  - Covers BOTH financial futures (S&P 500, NASDAQ, treasuries, currencies)
 *    AND commodities (gold, oil, ag, livestock).
 *  - Commercial vs non-commercial vs non-reportable positioning.
 *  - Released Fridays for the prior Tuesday's data.
 *
 * Most-watched signal in macro and futures trading. "Smart money" =
 * commercials (hedgers) — their positioning often presages major moves.
 */
...
export async function getRecentCOTReports(opts: { marketContains?: string; limit?: number } = {}): Promise<COTRow[]> {
  ...
  try {
    ...
    if (!res.ok) { console.error("[cftc] HTTP", res.status); return []; }
...
/** Convenience: signed net positioning (long minus short) for each trader category */
