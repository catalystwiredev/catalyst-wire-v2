 ...
 * Secrets: ALPACA-KEY-ID + ALPACA-SECRET-KEY (from Azure Key Vault)
...
async function getAlpacaCredentials() {
  const keyId = await getSecret("ALPACA-KEY-ID");
  const secret = await getSecret("ALPACA-SECRET-KEY");
  return { keyId, secret };
}
...
export async function getStockBars(symbol: string, opts: {
  ...
  const { keyId, secret } = await getAlpacaCredentials();

  if (!keyId || !secret) {
    console.warn("[alpaca] Missing credentials");
    return [];
  }
