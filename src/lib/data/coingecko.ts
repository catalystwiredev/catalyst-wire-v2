 ...
 * Secret: COINGECKO-API-KEY (from Azure Key Vault, optional)
...
async function getBase(): Promise<string> {
  try {
    const key = await getSecret("COINGECKO-API-KEY");
    return key ? PRO : BASE;
  } catch {
    return BASE; // free tier fallback
  }
}

async function headers(): Promise<Record<string, string>> {
  try {
    const k = await getSecret("COINGECKO-API-KEY");
...
export async function getCryptoPrices(coinIds: string[] = ["bitcoin","ethereum","solana","chainlink","uniswap","avalanche-2"]): Promise<CryptoPrice[]> {
  ...
  const res = await fetch(url, { next: { revalidate: 60 }, headers: await headers() });
  if (!res.ok) return [];
...
export async function getTrendingCoins(): Promise<{ id: string; name: string; symbol: string; market_cap_rank: number }[]> {
  const base = await getBase();
  const res = await fetch(`${base}/search/trending`, { next: { revalidate: 300 }, headers: await headers() });
  if (!res.ok) return [];
  const data = await res.json();
...
export async function getCryptoNews(limit = 10): Promise<CryptoNews[]> {
  const base = await getBase();
  const res = await fetch(`${base}/news?per_page=${limit}`, { next: { revalidate: 120 }, headers: await headers() });
  if (!res.ok) return [];
  const data = await res.json();
...
export async function getCoinOHLC(coinId: string, days: 1 | 7 | 14 | 30 = 7): Promise<[number,number,number,number,number][]> {
  ...
  const res = await fetch(url, { next: { revalidate: 300 }, headers: await headers() });
  if (!res.ok) return [];
  return res.json();
}
